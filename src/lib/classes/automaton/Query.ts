...
export class Query implements Raw.SerializeRaw, Raw.ToRaw<Raw.RawQuery> {
	 ...
	 * */
	static readonly fromRaw: Raw.FromRaw<Raw.RawQuery, Query> = (raw) => {
		return new Query(
			raw.query,
			raw.comment,
			raw.isPeriodic,
	 ...
	 * */
	static readonly deserializeRaw: Raw.DeserializeRaw<Query> = (input) => {
...
export class Queries implements Raw.SerializeRaw, Raw.ToRaw<Raw.RawQuery[]> {
	 ...
	 * */
	static readonly deserializeRaw: Raw.DeserializeRaw<Queries> = (input) => {
	 ...
	 * */
	static readonly fromRaw: Raw.FromRaw<Raw.RawQuery[], Queries> = (raw) => {
		return new Queries(
			raw.map((rawSingle) => {
