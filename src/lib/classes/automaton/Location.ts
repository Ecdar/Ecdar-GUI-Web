...
import { Nickname, Invariant, LocationType, Urgency, Raw } from "../automaton";
...
export class Location implements Raw.SerializeRaw, Raw.ToRaw<Raw.RawLocation> {
	 ...
	 * */
	static readonly fromRaw: Raw.FromRaw<Raw.RawLocation, Location> = (raw) => {
		return new Location(
			raw.id,
	 ...
	 * */
	static readonly deserializeRaw: Raw.DeserializeRaw<Location> = (input) => {
