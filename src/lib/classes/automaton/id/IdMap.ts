...
export abstract class IdSet<T extends number | string, C extends HasId<I>, I extends Id<T>, R>
	extends AutomatonClass<R>
{
	constructor(
		private idConstructor: new (rawId: number | string) => I, 
	){super()}

	private idMap = new Map<T extends string ? string : number | string, I>();

	private orderedMap: (C | undefined)[] = [];
	private unorderedMap = new Map<T extends string ? string : number | string, C>();
	...
	getNewId(rawId: T extends string ? string : number | string | undefined): I | undefined {
		if(!rawId){
	...
	add(member: C) {
		...
		if (idOrder !== undefined) {
			if (this.orderedMap[idOrder] !== undefined) {
				throw new TypeError(`The member "${member.id.toRaw()}" cannot be added because it already exists`);
			}
		} else {
	...
	update(member: C){
		...
		if (idOrder !== undefined) {
			if (this.orderedMap[idOrder] === undefined) {
				throw new TypeError(`The member "${member.id.toRaw()}" cannot be updated because it does not exist`);
			}
		} else {
			const rawId = member.id.toRaw();
